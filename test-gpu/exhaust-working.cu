...
struct validate
{
    ...
    validate(
        ...
        // thrust::device_vector<int> catLimit_,
    ...
    __host__ __device__ int operator()(const int &bin)
    {
        ...
        for (int i = 0; i < movieCount; i++)
        {
            ...
            // if (catLimit[movieCat[i] - 1] == 0)
            //     return 0;
            if (movieEnd[lastIndex] > movieStart[i])
                return 0;

            // catLimit[movieCat[i] - 1]--;
            lastIndex = i;
            result++;
        }
...
int main()
{
    ...
    // thrust::transform(devStart.begin(), devStart.end(), devEnd.begin(), devStart.begin(), sum_two());
    ...
    thrust::device_vector<int> validCombinations(endBin - startBin);

    thrust::transform(startBin, endBin, validCombinations.begin(), validateFunctor);

    // int maxMovies = 0;

    // int count = thrust::reduce(startBin, endBin, 0, validate_combination(movieCount, &maxMovies, devCatLimit, devStart, devEnd, devCat));
